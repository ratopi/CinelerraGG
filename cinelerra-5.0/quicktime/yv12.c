...
static int decode(quicktime_t *file, unsigned char **row_pointers, int track)
{
	...
	int64_t y_size, u_size, v_size, bytes;
	int result = 0;
	initialize(vtrack);
	...
	if(file->color_model == BC_YUV420P && 
			...
			file->out_h == height)
	{
		result = !quicktime_read_data(file, (char*)row_pointers[0], y_size);
		result = !quicktime_read_data(file, (char*)row_pointers[1], u_size);
		result = !quicktime_read_data(file, (char*)row_pointers[2], v_size);
	}
	else
	{
...
//	int64_t offset = quicktime_position(file);
	...
	if(file->color_model == BC_YUV420P)
	{
		result = !quicktime_write_data(file, (char*)row_pointers[0], y_size);
		if(!result) result = !quicktime_write_data(file, (char*)row_pointers[1], u_size);
		if(!result) result = !quicktime_write_data(file, (char*)row_pointers[2], v_size);
	}
	else
	{
		cmodel_transfer(0, 
